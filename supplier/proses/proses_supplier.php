...
if (empty($nama_supplier) || empty($alamat) || empty($no_hp)) {
    header('Location: supplier.php?message=' . urlencode('Semua field harus diisi'));
    exit;
}
...
if ($id_supplier > 0) {
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            header('Location: supplier.php?message=' . urlencode('Supplier berhasil diperbarui'));
        } else {
            header('Location: supplier.php?message=' . urlencode('Gagal memperbarui supplier'));
        }
        $stmt->close();
    } else {
        header('Location: supplier.php?message=' . urlencode('Error: ' . $conn->error));
    }
} else {
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            header('Location: supplier.php?message=' . urlencode('Supplier berhasil ditambahkan'));
        } else {
            header('Location: supplier.php?message=' . urlencode('Gagal menambahkan supplier'));
        }
        $stmt->close();
    } else {
        header('Location: supplier.php?message=' . urlencode('Error: ' . $conn->error));
